...
window.jTour = function (tourdata, options) {
	...
	var version = '1.2.1',
		...
		defaults = {
			...
			onHide: function () {},
			onStep: function () {},
			// CUSTOM MMS APP BITS HERE
			exposeOnBeforeShow: function () {},
			mmsOnBeforeShow: function () {},
			mmsOnShow: function () {},
			mmsOnBeforeHide: function () {},
			mmsOnHide: function () {},
			mmsOnStep: function () {}
		};
	...
	var settings = $.extend({}, defaults, options),
		...
		stepdefaults = {
			...
			// CUSTOM MMS APP BITS HERE
			exposeOnBeforeShow: settings.mmsOnBeforeShow,
			mmsOnBeforeShow: settings.mmsOnBeforeShow,
			mmsOnShow: settings.mmsOnShow,
			mmsOnBeforeHide: settings.mmsOnBeforeHide,
			mmsOnHide: settings.mmsOnHide,
			mmsOnStep: settings.mmsOnStep
		};

	// IMPORTANT FOR FUNCTIONS
	for (var step in tourdata) {

		var number = 100 * step/(tourdata.length-1);

		var percent = Math.round( number * 10 ) / 10;


		prevStep = tourdata[step-1];
		step = tourdata[step];

		step.percent = percent;

		for (var option in step) {
			if (option == 'expose' && step[option] == true) {
				//console.log('expose');
			}

			//step[option].percent = percent;

			//console.log(step[option].percent);

			if (typeof(step[option]) === 'string' && option == 'element') {
				for (var i in step[option].split(' ')) {
					if (step[option].split(' ')[i].indexOf('#sidebar-') === 0) {
						/*prevStep.mmsOnBeforeHide = function(e) {
							// hide all side nav items
							$('#sidebar h5').next('ul').each(function(i) {
								// toogle everything closed
								if ($(this).prev('h5').hasClass('show-less')) {
									$(this).prev('h5').removeClass('show-less').addClass('show-more');
									$(this).toggle(0);
								}
							});
						};*/
						step.wait = 1000;
						prevStep.mmsOnBeforeHide = function(e) {
							//var sidebarId = e.closest('[id^="sidebar-"]').attr('id');
							var sidebarId = "sidebar-inbox";
							// loop through and see which sidbar groups need to be hiden and which one to show
							$('#sidebar h5').next('ul').each(function(i) {
								// if is the sidebar nav group that needs to be shown and not shown, toggle open
								if ($(this).closest('li').attr('id') == sidebarId) {
									if ($(this).prev('h5').hasClass('show-more')) {
										$(this).prev('h5').removeClass('show-more').addClass('show-less');
										$(this).toggle(500);
									}
								// if everything else if not already closed, needs to be toggled close
								} else {
									if ($(this).prev('h5').hasClass('show-less')) {
										$(this).prev('h5').removeClass('show-less').addClass('show-more');
										$(this).toggle(500);
			...
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'none') {
				step[option] = function() { };
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'triggerClick') {
				step[option] = function(e) {
					e.trigger('click');
				};
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'dynamicText') {
				var text = step[option].split('|')[1];
				step[option] = function(e, p) {
					var l = text.length;
					e.html(text.substr(0, Math.round(l/100*p)));
				};
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'tagNetwork') {
				step[option] = function(e, p) {
					var text = 'Facebook';
					e.find('.tagit-new input').val(text.substr(0, Math.round(text.length/100*p)));
					e.find('.tagit-new input').trigger('keyup keypress keydown input');
				};
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'tagName') {
				step[option] = function(e, p) {
					var text = 'MarketMeSuite';
					e.find('.tagit-new input').val(text.substr(0, Math.round(text.length/100*p)));
					e.find('.tagit-new input').trigger('keyup keypress keydown input');
				};
			} else if (typeof(step[option]) === 'string' && step[option].split('|')[0] == 'toggleSideNav') {
				step[option] = function(e) {
					var sidebarId; // sidebar id that needs to be shown
					for (var i in e.split(' ')) {
						if (e.split(' ')[i].indexOf('#sidebar-') === 0) {
							sidebarId = e.split(' ')[i].substring(1);
						}
					}
					// loop through and see which sidbar groups need to be hiden and which one to show
					$('#sidebar h5').next('ul').each(function(i) {
						...
						// everything else if not already closed, needs to be toggled close
						} else {
							if ($(this).prev('h5').hasClass('show-less')) {
								$(this).prev('h5').removeClass('show-less').addClass('show-more');
								$(this).toggle(500);
	...
	function init() {
		...
		$('<div/>', {
			'class': prefix + 'custom_progress'
		}).html('<div class="progress_text">progress:</div><div class="' + prefix + 'custom_progress_bar"></div>').appendTo(box);

		//save the progressbar for later
		progress = box.find('.' + prefix + 'progress_bar');

		customProgress = box.find('.jTour_custom_progress.progress_bar');
	...
	function show(step) {
		...
		if (!last) {
			...
			last.mmsOnBeforeHide.call(api, last.element);
		}
		...
		box[options.animationOut](options.delayOut, function () {
			...
			timeout = setTimeout(function () {
				...
				content.html('<p class="wrapper">'+options.html+'</p>');
				...
				scroll(scrolltopos.x, scrolltopos.y, settings.scrollDuration, function () {
					...
					options.exposeOnBeforeShow.call(api, $element);
					options.mmsOnBeforeShow.call(api, $element);

					//$('.jTour_custom_progress_bar').css({'width':options.percent+'%'});
					if ($('#progress-bar-style').length === 0) {
						$('head').append('<style id="progress-bar-style">.jTour_custom_progress_bar:before,.jTour_custom_progress_bar:after{width:'+options.percent+'%;}</style>');
					} else {
						$('#progress-bar-style').remove();
						$('head').append('<style id="progress-bar-style">.jTour_custom_progress_bar:before,.jTour_custom_progress_bar:after{width:'+options.percent+'%;}</style>');
					}
					// $('.jTour_progress_bar::before').css({width:options.percent+'%'});
